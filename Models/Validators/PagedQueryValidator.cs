...
public class PagedQueryValidator : AbstractValidator<PagesQuery>
{
    ...
    private string[] allowedSortBy = { nameof(Question.Title), nameof(Question.Author), nameof(Question.CreationDate), nameof(Question.AuthorId) };
    ...
    public PagedQueryValidator()
    {
        ...
        RuleFor(x => x.PageSize).Custom((value, context) =>
        {
            if(!allowedPageSizes.Contains(value)) 
                context.AddFailure("PageSize", $"Page size is invalid, must be in {string.Join(",", allowedPageSizes)}.");
        });
        ...
        {
            RuleFor(x => x.SortAscending).NotNull();
        });
    }
}